...
#   - 0bda:8176 (RTL8188EU) USB ID: a valid 8188EU ID upstream omits
#     because mainline's in-tree r8188eu claims it; this kernel has no
#     in-tree r8188eu. driver_info IS the chip type in this driver, so the
#     ID must be in the static table (dynamic new_id can never bind).
...
# on every registered-adapter unbind, and lacks the 8176 USB ID.
fork_checks=(
	...
	'USB_VENDER_ID_REALTEK, 0x8176:os_dep/linux/usb_intf.c'
)
